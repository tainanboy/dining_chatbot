...
def searchYelp(term = 'breakfast', location='San Francisco, CA', cuisine='Japanese', dining_date=datetime.now(), dining_time="18:00", num_people=4):
    """Use the yelp API to get top 3 restaurants from user inputs.
    Example usage: searchYelp("new york", "curry", "today", "22:00", "4")
    Args:
        location (str): The location of the business to query.
        cuisine (str)
        dining_date (str) 
        ...
        num_people (str)
    Returns:
        list: a list of tuples: [("Sushi Nakazawa", "23 Commerce St"), ...].
    """
    SEARCH_LIMIT = 3
    res = []
    for _ in range(SEARCH_LIMIT):
        try:
            response = query_api(term, location, cuisine)
